...
export const Header = () => {
    ...
    return (
        <header
            className="sticky top-0 z-40 w-full select-none bg-surface transition-all duration-[var(--duration-smooth)] ease-[var(--ease-guway)]"
        >
            <div className="flex h-16 md:h-20 items-center gap-2 md:gap-4 px-4 md:px-8">
                ...
                <div className="flex-none lg:flex-1 flex items-center">
                    {/* Spacer untuk Hamburger Sidebar pada Mobile */}
                ...
                {/* RIGHT: System Status & Bell Actions */}
                <div className="flex-none lg:flex-1 flex items-center justify-end gap-2 md:gap-6 relative">
                    ...
                    {/* BELL INTERACTION SYSTEM */}
                    <div className="relative">
                        <Button
                            variant={isOpen ? "inset" : "expel"}
                            onClick={() => setIsOpen(!isOpen)}
                            className="relative w-10 h-10 md:w-11 md:h-11 !p-0 rounded-xl md:rounded-2xl hover-tactile shrink-0 transition-all duration-[var(--duration-gentle)] ease-[var(--ease-guway)]"
                        >
                            <Bell
                                size={18}
                                className={`
                                    transition-colors duration-[var(--duration-smooth)] 
                                    ${isOpen ? "text-[var(--theme-base)]" : "text-text-muted"}
                                `}
                            />
                            ...
                            <AnimatePresence>
                                {unreadCount > 0 && (
                                    <motion.span
                                        ...
                                        className="absolute -top-1 -right-1 md:top-0 md:right-0 min-w-[18px] h-[18px] px-1 rounded-full flex items-center justify-center text-[9px] font-black text-white border-2 border-surface z-10"
                                    >
                        ...
                        <AnimatePresence>
                            {isOpen && (
                                <motion.div
                                    ...
                                    /* SYNC: Framer Motion easing disesuaikan dengan CSS */
                                    transition={{
                                        duration: 0.4,
                                        ease: [0.4, 0, 0.2, 1]
                                    }}
                                    className="absolute right-0 mt-4 w-72 md:w-80 bg-surface shadow-neumorph rounded-[var(--border-radius)] border border-white/5 overflow-hidden z-50"
                                >
                                    {/* Dropdown Header */}
            ...
            {/* Backdrop Overlay with Sync Transition */}
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        initial={{ opacity: 0 }}
